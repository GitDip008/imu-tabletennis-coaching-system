"""
Real-time TT coaching app driven by raw IMU CSV.

Data flow:
  RawIMUStreamer thread
      ├─ calibrate skeleton controller (first CALIBRATION_FRAMES rows)
      ├─ process_row → UDP → Unity skeleton  (every frame)
      └─ push ("imu_row", row_dict) → event queue

  Main thread (Tkinter)
      ├─ poll queue every 30 ms
      ├─ SlidingWindowExtractor: accumulate raw frames
      ├─ every STEP_SIZE frames with full window: extract 34 features
      ├─ StrokePredictor.predict() → update UI panels
      └─ LLM report on demand
"""
...
# Add 1_new_test to path so SiriusCeption_unity_controller.pyd is importable
...
try:
    ...
    print("[WARN] SiriusCeption_unity_controller not found — skeleton disabled.")
...
PLAYBACK_SPEED      = 1.0     # 1.0 = real-time; 0.5 = half speed; 2.0 = 2× speed
...
class RawIMUStreamer(threading.Thread):
    """
    Reads a raw IMU CSV, optionally calibrates + streams skeleton to Unity,
    and pushes rows into the event queue for feature extraction + prediction.
    """
    ...
    def __init__(
        ...
        send_to_unity: bool = True,
        calibration_frames: int = CALIBRATION_FRAMES,
    ):
        super().__init__(daemon=True)
        self.csv_path           = csv_path
        self.q                  = event_queue
        self.send_to_unity      = send_to_unity and UNITY_AVAILABLE
        ...
        try:
            ...
            with open(self.csv_path, newline="") as f:
                ...
                if controller is not None:
                    self.q.put(("status", f"Calibrating skeleton ({self.calibration_frames} frames)…"))
                    controller.calibrate(reader, frames=self.calibration_frames)
                    self.q.put(("status", "Streaming IMU data…"))
                ...
                for row in reader:
                    ...
                    if sleep_s > 0:
                        adjusted = sleep_s / max(PLAYBACK_SPEED, 0.01)
                        time.sleep(adjusted)
...
class RealTimeApp(tk.Tk):
    ...
    def __init__(self):
        super().__init__()
        self.title(f"TT Coaching System — Player {SUBJECT_ID}  [Raw IMU Mode]")
        ...
        # Session state
        self.session_rows   = []   # list of dict{FEATURE_COLS} for LLM report
    ...
    def _build_ui(self):
        ...
        hdr.pack(fill=tk.X)
        tk.Label(
            hdr, text="TT Coaching System  (Raw IMU + Unity Skeleton)",
            font=("Helvetica", 15, "bold"),
            fg="white", bg=COLORS["accent"],
        ).pack(side=tk.LEFT, padx=20)

        unity_status = "Unity ON" if (UNITY_AVAILABLE and SEND_TO_UNITY) else "Unity OFF"
        tk.Label(
            hdr, text=f"Player {SUBJECT_ID}  |  {unity_status}",
            font=("Helvetica", 11),
        ...
        self.status_var = tk.StringVar(value="Ready — select a CSV and press Start Session")
        tk.Label(
            footer, textvariable=self.status_var,
            font=("Helvetica", 10), fg=COLORS["subtext"], bg=COLORS["bg"],
            wraplength=280, anchor="e",
        ).pack(side=tk.RIGHT, padx=10)

    def _panel(self, parent, title, col, row=0):
        frame = tk.LabelFrame(
            parent, text=f"  {title}  ",
    ...
    def _start_session(self):
        ...
        self._log(f"Session started — Player {SUBJECT_ID}  |  CSV: {pathlib.Path(self.csv_path).name}\n",
                  tag="header")
    ...
    def _end_session(self):
        ...
        self.status_var.set(f"Session ended — {len(self.session_preds)} predictions")
        self._log(f"\nSession ended — {len(self.session_preds)} predictions\n",
                  tag="header")
        ...
        try:
            while True:
                ...
                if msg_type == "imu_row":
                    ...
                    self.status_var.set(f"Error: {payload[:60]}")
        except queue.Empty:
            pass
    ...
    def _process_imu_row(self, row: dict):
        self._frame_count = getattr(self, "_frame_count", 0) + 1
        self.lbl_frames.config(text=f"Frames: {self._frame_count}")

        features = self._extractor.add_frame(row)
        if features is None:
            return  # window not full or not at a step boundary yet

        result = self.predictor.predict(features)

        # Store extracted features as a dict so session_df can be built for the LLM report
    ...
    # ── UI helpers (identical to realtimeapp.py) ──────────────────────────────
    ...
    def _update_live_panel(self, result: dict):
        ...
        self.conf_canvas.create_rectangle(0, 0, int(w * conf), 22, fill=color, outline="")
        ...
        for cname, prob in result["probabilities"].items():
            ...
            bar_bg.delete("all")
            bar_bg.create_rectangle(0, 0, int(bw * prob), 14,
    ...
    def _update_stats_panel(self):
        ...
        total_strokes = sum(v for k, v in self.stroke_counts.items() if k != "No Stroke")
        ...
        for name, (bar_bg, lbl, color) in self.dist_bars.items():
            ...
            bw = bar_bg.winfo_width()
            bar_bg.delete("all")
            bar_bg.create_rectangle(0, 0, int(bw * cnt / max_count), 16,
                                    fill=color, outline="")
        ...
        stroke_idx = [i for i, p in enumerate(self.session_preds) if p["label_id"] != 0]
        if len(stroke_idx) > 1:
    ...
    def _log_event(self, idx: int, result: dict):
        ...
        self._log(f"[{idx:04d}]  {real:<22}  conf={conf:.2%}\n", tag=name)
